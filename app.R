...
test1$continent <- countrycode(
  sourcevar = test1[, "Country"],
  origin = "country.name",
...
server <- function(input, output, session) {
  ...
  output$Male1 <- renderPlot({
    if (req(input$slidering1) == "Low") {
      ggscatterhist(
        test6,
        x = "Human_DI", y = "Gross_National_Income_Per_Capita",
  ...
  k <- na.omit(k)
  test$continent <- countrycode(
    sourcevar = test[, "Country"],
    origin = "country.name",
    destination = "continent"
  )
  mm <- leaflet() %>%
    ...
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Gender Development Index Value (2019)`) * (10),
      fillOpacity = 0.7, color = "red", group = "GDI Value"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Human Development Index Female (value) (2019)`) * (10),
      fillOpacity = 0.7, color = "purple", group = "HDI Female value"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Human Development Index Male (value) (2019)`) * (10),
      fillOpacity = 0.7, color = "green", group = "HDI Male value"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Expected years of schooling Female (years) (2019)`) / (2),
      fillOpacity = 0.7, color = "goldenrod", group = "years of schooling Female"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Expected years of schooling Male (years) (2019)`) / (2),
      fillOpacity = 0.7, color = "cornflowerblue", group = "years of schooling Male"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Life expectancy at birth Female (years) (2019)`) / (10),
      fillOpacity = 0.7, color = "yellow", group = "Life expectancy at birth Female"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Life expectancy at birth Male (years) (2019)`) / (10),
      fillOpacity = 0.7, color = "pink", group = "Life expectancy at birth Male"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Mean years of schooling Female (years) (2019)`) * (2 / 3),
      fillOpacity = 0.7, color = "orange", group = "Mean years of schooling Female"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Mean years of schooling Male (years) (2019)`) * (2 / 3),
      fillOpacity = 0.7, color = "darkgreen", group = "Mean years of schooling Male"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Estimated  gross national income per capita Female (2017 PPP $) (2019)`) / (5000),
      fillOpacity = 0.7, color = "palevioletred", group = "gross national income per capita Female"
    ) %>%
    addCircleMarkers(
      data = k, lat = ~y, lng = ~x, weight = 0.1, radius = ~ (k$`Estimated  gross national income per capita Male (2017 PPP $) (2019)`) / (5000),
      fillOpacity = 0.7, color = "darkred", group = "gross national income per capita Male"
    ) %>%
    addLayersControl(
      position = "bottomright",
      overlayGroups = c(
        "GDI Value", "HDI Female value", "HDI Male value", "years of schooling Female",
    ...
    hideGroup(c(
      "HDI Female value", "HDI Male value", "years of schooling Female",
      "years of schooling Male", "Life expectancy at birth Female", "Life expectancy at birth Male",
      "Mean years of schooling Female", "Mean years of schooling Male", "gross national income per capita Female",
      "gross national income per capita Male"
    ))
  ...
  output$plot1 <- renderPlot({
    if (input$select == "World") {
      ...
      ci.band <- predict(model,
        newdata = x,
        interval = "confidence", level = 0.95
      )
      pi.band <- predict(model,
        newdata = x,
      ...
      plot1 <- test %>%
        ...
        coord_cartesian(xlim = c(min(test$Mean_Years_of_Schooling), max(test$Mean_Years_of_Schooling)), ylim = c(min(test$Gross_National_Income_Per_Capita), max(test$Gross_National_Income_Per_Capita))) +
      ...
      names(x)[1] <- "Mean_Years_of_Schooling"
      ci.band <- predict(modeling,
        newdata = x,
        interval = "confidence", level = 0.95
      )
      pi.band <- predict(modeling,
        newdata = x,
        interval = "prediction", level = 0.95
      )
      ...
      plot1 <- t %>%
        ggplot(aes(x = Mean_Years_of_Schooling, y = Gross_National_Income_Per_Capita)) +
        geom_point(color = "blue") +
        coord_cartesian(xlim = c(min(t$Mean_Years_of_Schooling), max(t$Mean_Years_of_Schooling)), ylim = c(min(t$Gross_National_Income_Per_Capita), max(t$Gross_National_Income_Per_Capita))) +
        geom_smooth(method = "lm", col = "green")
      plot1 <- plot1 + geom_line(x, mapping = aes(x = x$Mean_Years_of_Schooling, y = ci.band$lwr), colour = "purple")
      plot1 <- plot1 + geom_line(x, mapping = aes(x = x$Mean_Years_of_Schooling, y = ci.band$upr), colour = "purple")
      plot1 <- plot1 + geom_line(x, mapping = aes(x = x$Mean_Years_of_Schooling, y = pi.band$lwr), colour = "orange")
